...
from digits_classification import networks
from digits_classification.lib.misc import random_pairs_of_minibatches
...
class ERM(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(ERM, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.featurizer = networks.MNIST_CNN()
        self.classifier = nn.Linear(4*64, num_classes)
    ...
    def updata_with_masking(self, minibatches, threshold=0.7):
        all_x = torch.cat([x for x,y in minibatches])
        all_y = torch.cat([y for x,y in minibatches])
        prediction=self.predict(all_x)
        mask=F.softmax(prediction).ge(threshold)
        for i,row in enumerate(mask):
            if True in row:
                mask[i]=torch.ones_like(mask[i]).float()
        prediction.register_hook(lambda grad: grad*mask)
        loss = F.cross_entropy(prediction, all_y)
...
class MinMax(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams, vae):
        super(MinMax, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.num_domains=num_domains
        self.featurizer = networks.Featurizer(input_shape, self.hparams)
        self.classifier = nn.Linear(self.featurizer.n_outputs, num_classes)
        self.network = nn.Sequential(self.featurizer, self.classifier)
        self.code_generator=nn.Sequential(nn.Linear(self.num_domains),nn.softmax(dim=1))
        self.vae=vae
        ...
        self.gen_optim=torch.optim.Adam(self.code_generator.parameters(),lr=self.hparams["lr"],weight_decay=self.hparams['weight_decay'])

    def update(self, minibatches, unlabeled=None):
        z=torch.randn(self.num_domains).unsqueeze(0).cuda()
        all_x = torch.cat([x for x,y in minibatches])
        all_y = torch.cat([y for x,y in minibatches])

        new_x=self.domain_transfer(all_x,code)


        loss = F.cross_entropy(self.predict(new_x), all_y)
