...
from dlow_model import BaseNetwork as Base
...
class InterpolationGAN(nn.Module):
    ...
    def __init__(self, conf, device, is_train=True):
        ...
        if is_train:
            ...
            self.ans_real = Variable(torch.ones([self.conf.batch_size, 1, 3, 3]), requires_grad=False).to(device)
            self.ans_fake = Variable(torch.zeros([self.conf.batch_size, 1, 3, 3]), requires_grad=False).to(device)
    ...
    def set_input(self, input, epoch):
        ...
        zero_tensor=torch.zeros((1,1)).cuda()
        if epoch < (1/2*self.conf.train_iters):
            self.domainess = utils.get_single_z(epoch, self.conf.train_iters/2,1).to(self.device)
            z1=0.5*torch.rand(1).cuda()*(1-self.domainess)

            self.domainess = torch.cat([1-self.domainess-z1,self.domainess,z1],dim=1)
            self.z = self.domainess.view(1,3,1,1).repeat(1,8,1,1)


        else:
            self.domainess = utils.get_single_z(epoch-self.conf.train_iters/2, self.conf.train_iters/2,1).to(self.device)

            z1=0.5*torch.rand(1).cuda()*(1-self.domainess)
            self.domainess = torch.cat([z1,1-self.domainess-z1,self.domainess],dim=1)
            self.z = self.domainess.view(1,3,1,1).repeat(1,8,1,1)
