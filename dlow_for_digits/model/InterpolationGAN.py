...
class InterpolationGAN(nn.Module):
    ...
    def forward(self, input, domainess):
        ...
        Z = d.view(1,2,1,1).repeat(1,8,1,1)                       
    ...
    def train(self):
        ...
        # Train G
        self.fake_T = self.G_ST(self.real_S, self.z)                                            
        self.recons_S = self.G_TS(self.fake_T, self.z)                                          
        ...
        self.recons_T1 = self.G_ST(self.fake_S1, self.z1)                                        
        ...
        # Train D
        loss_S_real_S = self.criterionGAN(self.D_S(self.real_S), self.ans_real)                

        loss_S_fake_S1 = self.criterionGAN(self.D_S(self.fake_S1.detach()), self.ans_fake)       
        self.loss_D_S1 = loss_S_real_S + loss_S_fake_S1

        loss_S_fake_S2 = self.criterionGAN(self.D_S(self.fake_S2.detach()), self.ans_fake)       
        self.loss_D_S2 = loss_S_real_S + loss_S_fake_S2

        loss_S_fake_S3 = self.criterionGAN(self.D_S(self.fake_S3.detach()), self.ans_fake)       
